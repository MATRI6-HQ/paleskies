/* Paleskies interactions: deliberately small, progressively enhanced.
   Shared by index.html, 404.html and the legal pages, so every lookup is optional:
   a page that omits a block simply skips that behaviour. */
(() => {
  ...
  // Nav: mark the section currently in view. Tracks the topmost section whose start
  // has passed the header, so short sections at the end of the page still register.
  const navLinks = [...document.querySelectorAll('.site-nav a[href^="#"]')];
  if (navLinks.length) {
    const targets = navLinks
      .map((link) => ({ link, section: document.querySelector(link.getAttribute('href')) }))
      .filter((entry) => entry.section);
    let isNavUpdateQueued = false;
    const markCurrent = () => {
      isNavUpdateQueued = false;
      const line = window.scrollY + 140; // just under the fixed header
      const atBottom = window.innerHeight + window.scrollY >= document.body.scrollHeight - 2;
      let current = targets[0];
      targets.forEach((entry) => {
        if (entry.section.getBoundingClientRect().top + window.scrollY <= line) current = entry;
      });
    ...
    const requestNavUpdate = () => {
      if (!isNavUpdateQueued) {
        isNavUpdateQueued = true;
